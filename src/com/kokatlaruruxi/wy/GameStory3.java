...
public class GameStory3 extends Module{
	...
	private final String ImagePath[] = {"stroy/P13", "stroy/P11", "stroy/P12"};
		
	private int nextWaitingTime;
	
	private final int imageCoordinate[] = {26, 97, 26, 520, 273, 630};
	
	//移动方式1：从上往下，2：从下往上，3：从左往右，4：从右往左，0：不动，5：从小放大
	private final byte moveDirect[] = {1, 2, 5};
	...
	public void paint(Canvas canvas) {
		...
		{			
			...
			stroyImage.get(i).drawBitmap(canvas, stroyImage.get(i).x, stroyImage.get(i).y, stroyImage.get(i).size, stroyImage.get(i).size, stroyImage.get(i).Alpha, 0, 0, 0, 0, 0, 0);
		}
	}
...
//			}			
		...
		else if(moveDirect[screenLayerNextImage]==5)
		{			
//			if(screenLayerNextImage==3)
//			{
//				nextWaitingTime --;
...
//			else
//			{			
			if(stroyImage.get(screenLayerNextImage).size<1.2f&&
			   stroyImage.get(screenLayerNextImage).size!=1f)
			{
				stroyImage.get(screenLayerNextImage).size += 0.2f;
				
				if(stroyImage.get(screenLayerNextImage).size>1.2)
				{
					stroyImage.get(screenLayerNextImage).size = 1f;
				}
			}
