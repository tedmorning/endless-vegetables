...
//	private Sprite starGame;
	...
	{
		uiProgress1 = new Sprite(GameImage.getImage("newui/ui_progress_01"));
		
		uiProgress2 = new Sprite(GameImage.getImage("newui/ui_progress_02"));
		
		uiProgress3 = new Sprite(GameImage.getImage("newui/progress_3"));
		
		butPasue = new Sprite(GameImage.getImage("newui/ui_pause_01"));
...
//		userHelp = new Sprite(GameImage.getImage("newui/icon_photo_01"));
		...
		if(UserRequest.getUser().getFriendIconID()!=null)
		{
			...
			else
			{
				...
				if(player.getIcon()==null)
				{
					...
					userHelp = new Sprite(GameImage.getImage("newui/icon_photo_02"));
				}
				else
				{
					//已经从网络拿到用户头像
		...
		icon1 = new Sprite(GameImage.getImage("newui/icon_01"));
		
		icon2 = new Sprite(GameImage.getImage("newui/icon_02"));
		
		icon11 = new Sprite(GameImage.getImage("newui/icon_01_1"));
		
		icon21 = new Sprite(GameImage.getImage("newui/icon_02_1"));
		
		goldGame = new Sprite(GameImage.getImage("newui/Gold_01"));
		
//		starGame = new Sprite(GameImage.getImage("newui/Star_01"));
		
		stoneGame = new Sprite(GameImage.getImage("newui/Gem_01"));
		
		numScoreBit = GameImage.getAutoSizecutBitmap("newui/num_01", 11, 1, GameImage.Sort_line);
		
		numScore =new Sprite[numScoreBit.length];
		
		for(int i=0;i<numScoreBit.length;i++)
		{
			numScore[i]=new Sprite(numScoreBit[i]);
		}
		
		numScoreBit2 = GameImage.getAutoSizecutBitmap("newui/num_02", 10, 1, GameImage.Sort_line);
		
		numScore2 =new Sprite[numScoreBit2.length];
		
		for(int i=0;i<numScoreBit2.length;i++)
		{
			numScore2[i]=new Sprite(numScoreBit2[i]);
		}
		
		itemNumBit = GameImage.getAutoSizecutBitmap("newui/icon_01_num", 10, 1, GameImage.Sort_line);
		
		itemNum =new Sprite[itemNumBit.length];
		
		for(int i=0;i<itemNumBit.length;i++)
		{			
			itemNum[i]=new Sprite(itemNumBit[i]);
		}
		
		itemNum1Bit = GameImage.getAutoSizecutBitmap("newui/icon_02_num", 10, 1, GameImage.Sort_line);
		
		itemNum1 =new Sprite[itemNum1Bit.length];
		
		for(int i=0;i<itemNum1Bit.length;i++)
		{
			itemNum1[i]=new Sprite(itemNum1Bit[i]);
		}		
...
//		starGame.recycleBitmap();
		...
		for(int i=0;i<numScore2.length;i++)
		{
			GameImage.delImage(numScore2[i].bitmap);
			
			numScore2[i].recycleBitmap();
		}
		...
		GameImage.delImageArray(numScoreBit2);	
	}
...
//				else if(item.level==3)
			...
			item.tool = new Sprite(GameImage.getImage(imgId[item.type][item.level-1]));
...
//		setGameTime(90);//设置游戏时间
	...
	public void paint(GameMain gameMain, Canvas canvas, int gameNumber)
	{						
		//------------------------------------- 上中  ------------------------------------------------			
...
//		GameLibrary.DrawNumber(canvas, numScore, starGame.bitmap.getWidth() + 4*GameConfig.f_zoomx, 9*GameConfig.f_zoomy, wordNumChars, ""+gameNumber, paint, GameLibrary.TL, 0);
		...
		GameLibrary.DrawNumber(canvas, numScore2, 4*GameConfig.f_zoomx, 9*GameConfig.f_zoomy, itemNumChars, ""+gameNumber, paint, GameLibrary.TL, 0);
		
		goldGame.drawBitmap(canvas,goldGame.bitmap, 5*GameConfig.f_zoomx, goldGame.bitmap.getHeight() + (10+8)*GameConfig.f_zoomy, paint);	
		
		GameLibrary.DrawNumber(canvas, numScore, goldGame.bitmap.getWidth() + 8*GameConfig.f_zoomx, goldGame.bitmap.getHeight() + (14+8)*GameConfig.f_zoomy, wordNumChars, ""+goldNumber, paint, GameLibrary.TL, 0);		
		
		stoneGame.drawBitmap(canvas,stoneGame.bitmap, 5*GameConfig.f_zoomx, goldGame.bitmap.getHeight() + goldGame.bitmap.getHeight() + (15+8)*GameConfig.f_zoomy, paint);	
		
		GameLibrary.DrawNumber(canvas, numScore, stoneGame.bitmap.getWidth() + 8*GameConfig.f_zoomx, goldGame.bitmap.getHeight() + goldGame.bitmap.getHeight() + (19+8)*GameConfig.f_zoomy, wordNumChars, ""+gemNumber, paint, GameLibrary.TL, 0);		
		
		//---------------------------------------- 时间 ------------------------------------------
		
		GameLibrary.DrawNumber(canvas, numScore, (GameConfig.GameScreen_Width - GameConfig.GameScreen_Width/4)+22*GameConfig.f_zoomx, 14*GameConfig.f_zoomy, wordNumChars, minute+":"+second, paint, GameLibrary.TH, 0);								
	}		
...
//		float y = GameConfig.GameScreen_Height-icon1.bitmap.getHeight()-10*GameConfig.f_zoomy;
	...
	private void itemSkill(GameMain gameMain, Item item, MotionEvent event)
	{
		switch(item.type)
		{
			case ITEM_TYPE_1:				
