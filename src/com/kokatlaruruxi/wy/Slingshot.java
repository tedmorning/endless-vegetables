...
public class Slingshot {
	...
	private Sprite slingShotPiece;
	
	private Sprite slingShotPieceFrameLeft;
...
//	private int slingShotRopeYArray[] = {6,-16,-12,-8};
	...
	{
		//弹弓皮图片
		slingShotPiece = new Sprite(GameImage.getImage(slingShotPieceImageArray[slingShotLevel*2]));
		
		//左侧弹弓支架
//		slingShotPieceFrameLeft = new Sprite(GameImage.getImage(slingShotPieceImageArray[slingShotLevel*2+1]));
		slingShotPieceFrameLeft = new Sprite();
		slingShotPieceFrameLeft.initSprite(CoolEditDefine.SLINGSHOT1+slingShotLevel, 0, 0, Sprite.SPRITE_STATE_NORMAL);
		slingShotPieceFrameLeft.changeAction(0);
		
		SpriteLibrary.loadSpriteImage(CoolEditDefine.SLINGSHOT1+slingShotLevel);
...
//		comboPrompt = new Sprite(GameImage.getImage("newCombo/ui_hold"));
	...
	{	
		...
		SpriteLibrary.DelSpriteImage(CoolEditDefine.SLINGSHOT1+slingShotLevel);
...
//		slingShotPieceFrameRight.recycleBitmap();		
	 ...
	 * 改变弹射架状态
	 * */
	public void changeSlingShotPieceFrame(int _state)
	{
		slingShotPieceFrameLeft.changeAction(_state);
	}
...
//		slingShotLevel = 0;
		...
		slingShotRope_y = (int)(slingShotRopeYArray[slingShotLevel]*GameConfig.f_zoomy);
...
//		slingShotPieceFrameLeft.drawBitmap(canvas, slingShotPieceFrameLeft.bitmap, SLINGSHOT_X - slingShotPieceFrameLeft.bitmap.getWidth()/2, SLINGSHOT_Y - 30*GameConfig.f_zoom, paint);
		...
		slingShotPieceFrameLeft.paintSprite(canvas, SLINGSHOT_X, (int)(SLINGSHOT_Y - 30*GameConfig.f_zoomy));
		...
		gameMain.waitingSpriteBulletRight.paint(canvas, GameMonster.shxSkillTime>0?true:false);
...
//		//------------------------------------- 气积满状态 ---------------------------------------------
//		comboPrompt.drawBitmap(canvas, SLINGSHOT_X-comboPrompt.bitmap.getWidth()/2, SLINGSHOT_Y+120*GameConfig.f_zoomy, 1f, 1f, 255, 0, 0, 0, 0, 0, 0);
	...
	{
		slingShotPieceFrameLeft.updataSprite();
...
//				indicator.setSnipeState(false);
	...
	public void recover(GameMain gameMain)//弹弓还原
	{
//		//教学
//		if(gameMain.gameTeaching.pauseState())
//		{												
...
//			indicator.setSnipeState(false);						
	...
	private void countSendBulletNumber(GameMain gameMain)
	{
