...
public class GameMonster {
	...
	public void init()
	{
		...
		gameMonsterMove = new GameMonsterMove();
		...
		gameShield = new GameShield();
		
		gameSlow = new GameSlow();
		
		gameHeadFire = new GameHeadFire();
		
		gameRoseThorn = new GameRoseThorn();
	...
	public void changeGameRoseThorn(int id, int actionName)
	{
		gameRoseThorn.changeState(id, actionName);
	}
...
//	}
			             ...
			             int moveSpeed, int attackTime, int standWaitingTime)
	{
		...
		sprite.initSprite(kind, x, y, Sprite.SPRITE_STATE_NONE);
...
//		sprite.byMoveStyle = (byte)MoveStyle;
		...
		sprite.lifeMax = bloodIsOne?1:SpriteLibrary.GetHP(sprite.kind);
		...
		gameSlow.setSlow((int)sprite.x, (int)sprite.y-SpriteLibrary.GetH(kind));
		
		gameHeadFire.setHeadFire((int)sprite.x, (int)sprite.y-SpriteLibrary.GetH(kind));
		
		gameRoseThorn.setRoseThorn((int)sprite.x, (int)sprite.y);
	...
	public ArrayList<Sprite> getEnemyList(int kind)
	{
	...
	public void paint(Canvas canvas, GameMain gameMain)
	{
		...
		{			
			if(Enemy.get(EnemySort[i]).getActionName()!=Sprite.Enemy_action02)
			{				
				if(Enemy.get(EnemySort[i]).getActionName()>Sprite.Enemy_action0&&
				   ...
				   Enemy.get(EnemySort[i]).kind!=CoolEditDefine.Enemy_YGDS)
				{
					...
					{		
						if(Enemy.get(EnemySort[i]).kind==CoolEditDefine.Enemy_MEGICWATER&&Enemy.get(EnemySort[i]).state==Sprite.SPRITE_STATE_STAND)
...
//		gameMain.gameRose.clearPaint();
		...
		for(int i=0;i<EnemySort.length;i++)
		{
			if(Enemy.get(EnemySort[i]).kind != CoolEditDefine.Enemy_MMJS&&
			   ...
			   Enemy.get(EnemySort[i]).kind != CoolEditDefine.Enemy_SIREN)
			{
//				gameMain.gameRose.paint(canvas, (int)Enemy.get(EnemySort[i]).y);
				...
				gameSlow.paint(canvas, EnemySort[i], (int)Enemy.get(EnemySort[i]).x, (int)Enemy.get(EnemySort[i]).y-SpriteLibrary.GetH(Enemy.get(EnemySort[i]).kind));
				
				if(Enemy.get(EnemySort[i]).isHeadFire)
//				if(Enemy.get(EnemySort[i]).lostBloodTime>0)
				gameHeadFire.paint(canvas, EnemySort[i], (int)Enemy.get(EnemySort[i]).x, (int)Enemy.get(EnemySort[i]).y-SpriteLibrary.GetH(Enemy.get(EnemySort[i]).kind));
				
				if(Enemy.get(EnemySort[i]).roseThornTime>0)
				gameRoseThorn.paint(canvas, EnemySort[i], (int)Enemy.get(EnemySort[i]).x, (int)Enemy.get(EnemySort[i]).y);
			}
		}
		
//		gameMain.gameRose.paint(canvas, 0);
			...
			{										
				...
				{					
					...
					for(int j=0;j<Enemy.size();j++)
					{
						...
						{
							if(Enemy.get(j).state!=Sprite.SPRITE_STATE_NONE&&
							   Enemy.get(j).state!=Sprite.SPRITE_STATE_NORMAL&&
							   Enemy.get(j).state!=Sprite.SPRITE_STATE_DEAD&&
							   Enemy.get(j).state!=Sprite.SPRITE_STATE_REVIVAL)
							{
		...
		for(int i=0;i<Enemy.size();i++)
		{
			...
			if(Enemy.get(i).transitionWaiting>0)
			{
				   ...
				   Enemy.get(i).kind != CoolEditDefine.Enemy_CHG)
				{
					if(Enemy.get(i).transitionWaiting == 0)
					{
						Enemy.get(i).changeAction(Sprite.Enemy_action0);
						
						Enemy.get(i).setState(Sprite.SPRITE_STATE_NORMAL);	
...
//							}
				...
				{		
					if(Enemy.get(i).dizzinessTime>0)//眩晕
					...
					{						
						...
						gameRoseThorn.updata(i);
						
						if(Enemy.get(i).state == Sprite.SPRITE_STATE_MOVE||
						   Enemy.get(i).state == Sprite.SPRITE_STATE_STAND||
						   Enemy.get(i).state == Sprite.SPRITE_STATE_ATTACK||
						   Enemy.get(i).state == Sprite.SPRITE_STATE_REVIVAL)
						{
					...
					else 
					{
						...
						if(Enemy.get(i).state != Sprite.SPRITE_STATE_NONE)
						Enemy.get(i).updataSprite();												
					}
				}	
			}
		...
		for(int i=0;i<Enemy.size();i++)
		{
			if(Enemy.get(i).state==Sprite.SPRITE_STATE_NONE && Enemy.get(i).life<=0)
			{
...
//		}
		...
		{
			if(sprite.state == Sprite.SPRITE_STATE_STAND)
			{
					...
					if(!getEnemyList().get(i).isFly)
					{
						...
						{									
							if(ExternalMethods.RectCollision(sprite.getCollisionRect(), getEnemyList().get(i).getCollisionRect()))
							{
								...
								sprite.changeAction(Sprite.Enemy_action27);
				...
				if(sprite.attackTime>0)
				{
					...
					{						
						...
						if(sprite.byMoveDirect==0)
						{
							...
							sprite.changeAction(Sprite.Enemy_action19);
						}
						else
						{														
							gameMonsterMove.RToLMove(sprite);
							
							sprite.changeAction(Sprite.Enemy_action18);
						}
...
//							sprite.changeAction(Sprite.Enemy_action03);
					...
					{
						if(sprite.state!=Sprite.SPRITE_STATE_ATTACK)
						{
							if(sprite.byMoveDirect==0)
							{
								sprite.changeAction(Sprite.Enemy_action05);
							...
							{
								sprite.changeAction(Sprite.Enemy_action17);
							}
				...
				else 
				{
					...
					{
						if(sprite.getActionName()!=Sprite.Enemy_action04)
							sprite.changeAction(Sprite.Enemy_action04);
					}
					else
					{
						if(sprite.getActionName()!=Sprite.Enemy_action16)
							sprite.changeAction(Sprite.Enemy_action16);
				...
				sprite.kind == CoolEditDefine.Enemy_YGDS)//地鼠系列行动方式
		{
			...
			{	
				...
				if(gameMonsterMove.LongitudinalMove(sprite))
				{
					if(sprite.state!=Sprite.SPRITE_STATE_GROUND)
					{
						sprite.changeAction(Sprite.Enemy_action07);
						
						sprite.setState(Sprite.SPRITE_STATE_GROUND);	
			...
			{		
				if(sprite.y!=sprite.orgbyMoveStop)
				{
					if(sprite.byMoveWaitingTime>0)
					{
						...
						if(sprite.byMoveWaitingTime==0)
						{
							if(sprite.state!=Sprite.SPRITE_STATE_BURROW)
							{
								sprite.changeAction(Sprite.Enemy_action06);
								
								sprite.setState(Sprite.SPRITE_STATE_BURROW);
...
//							sprite.setState(Sprite.SPRITE_STATE_ATTACK);	
							...
							if(sprite.y != sprite.orgbyMoveStop)
							{
								...
								while(rmd>0)
								{
									...
									addEnemy(CoolEditDefine.Enemy_YGXTM, ExternalMethods.throwDice(SpriteLibrary.GetW(CoolEditDefine.Enemy_YGXTM)/2, GameConfig.GameScreen_Width-SpriteLibrary.GetW(CoolEditDefine.Enemy_YGXTM)/2), 
...
//						}
		...
		{		
			if(sprite.state==Sprite.SPRITE_STATE_REVIVAL)
			{
				sprite.revivalTime --;
			...
			else if(sprite.state==Sprite.SPRITE_STATE_MOVE)
			{				
				if(gameMonsterMove.LongitudinalMove(sprite))
				{
					if(sprite.y!=sprite.orgbyMoveStop)
		...
		{						
			...
			{											
				...
				else if(sprite.attackTime!=-100)
				{
					...
					{
						if(sprite.state!=Sprite.SPRITE_STATE_VIOLENT)
						{
							sprite.changeAction(Sprite.Enemy_action08);
							
							sprite.setState(Sprite.SPRITE_STATE_VIOLENT);
					...
					{
						if(sprite.state!=Sprite.SPRITE_STATE_SUMMON)
						{
							sprite.changeAction(Sprite.Enemy_action09);
							
							sprite.setState(Sprite.SPRITE_STATE_SUMMON);	
...
//									 sprite.orgbyMoveStop, 1, ExternalMethods.throwDice(100, 200), 10);
					...
					{
						if(sprite.state!=Sprite.SPRITE_STATE_RESTORE)
						{
							sprite.changeAction(Sprite.Enemy_action10);
							
							sprite.setState(Sprite.SPRITE_STATE_RESTORE);	
							
							effect.add(CoolEditDefine.Effect_RESTORE, (int)sprite.x, (int)sprite.y, Sprite.SPRITE_STATE_NORMAL, 0, 1f);
		...
		{			
			...
			else
			{
				...
				if(sprite.byMoveWaitingTime==0)
				{
					...
					if(sprite.byMoveDirect == 0)//从右向左
					{
						sprite.changeAction(Sprite.Enemy_action11);
					...
					{
						sprite.changeAction(Sprite.Enemy_action13);
			...
			{											
				...
				{					
					...
					else if(rmd==1)//召唤第一场景怪物
					{
						if(sprite.state!=Sprite.SPRITE_STATE_SUMMON)
						{
							sprite.changeAction(Sprite.Enemy_action22);
					...
					else if(rmd==2)//召唤触手
					{
						if(sprite.state!=Sprite.SPRITE_STATE_SUMMON)
						{
							...
							while(num>0)
							{
								...
								addEnemy(CoolEditDefine.Enemy_SHZYCS, ExternalMethods.throwDice(SpriteLibrary.GetW(CoolEditDefine.Enemy_SHZYCS)/2, GameConfig.GameScreen_Width-SpriteLibrary.GetW(CoolEditDefine.Enemy_SHZYCS)/2), 
		...
		{					
			...
			{											
				...
				else if(sprite.attackTime!=-100)
				{
					...
					{
						if(sprite.state!=Sprite.SPRITE_STATE_SUMMON && !centerMove)
						{
							sprite.changeAction(Sprite.Enemy_action24);
					...
					else if(rmd==2)//鬼魅尖叫
					{
						if(sprite.state!=Sprite.SPRITE_STATE_SUMMON)
						{
							sprite.changeAction(Sprite.Enemy_action25);
					...
					else if(rmd==3)//召唤
					{
						if(sprite.state!=Sprite.SPRITE_STATE_SUMMON)
						{
							sprite.changeAction(Sprite.Enemy_action26);
...
//									 sprite.orgbyMoveStop, 2, 50, ExternalMethods.throwDice(25, 75));
		...
		else if(sprite.kind == CoolEditDefine.Enemy_YGYKS)//月影鹰口水行动方式
		{			
			if(sprite.state==Sprite.SPRITE_STATE_ATTACK)
			{
				if(gameMonsterMove.LongitudinalMove(sprite))
	...
	public void collision(GameMain gameMain, Sprite bullet)	
	{
		int i = 0;
		...
		for(;i<Enemy.size();i++)
		{
			...
			{
				int attackNumber = SpriteLibrary.GetAttack(bullet.kind);
				...
				{
					if(Enemy.get(i).state == Sprite.SPRITE_STATE_NORMAL||
					   Enemy.get(i).state == Sprite.SPRITE_STATE_STAND||				   
					   Enemy.get(i).state == Sprite.SPRITE_STATE_ATTACK||
					   Enemy.get(i).state == Sprite.SPRITE_STATE_SUMMON||
					   Enemy.get(i).state == Sprite.SPRITE_STATE_REVIVAL||
					   Enemy.get(i).state == Sprite.SPRITE_STATE_INJURE||
					   Enemy.get(i).state == Sprite.SPRITE_STATE_GROUND||
					   Enemy.get(i).state == Sprite.SPRITE_STATE_VIOLENT||
					   Enemy.get(i).state == Sprite.SPRITE_STATE_RESTORE||
					   (Enemy.get(i).kind!=CoolEditDefine.Enemy_DS&&
					    Enemy.get(i).kind!=CoolEditDefine.Enemy_YGDS&&
					    Enemy.get(i).state == Sprite.SPRITE_STATE_MOVE)||
					    (Enemy.get(i).dizzinessTime>0&&Enemy.get(i).life>0))
					{
							...
							if(!bullet.isTouch)
							{
								if(Enemy.get(i).kind==CoolEditDefine.Enemy_SHHM)
								{
									   ...
									   bullet.getActionName()==11)
									{
										...
										Enemy.get(i).life -= attackNumber;
										
										Enemy.get(i).isGlide = false;
						...
						{							
							Enemy.get(i).changeAction(Sprite.Enemy_action01);
							
							Enemy.get(i).setState(Sprite.SPRITE_STATE_INJURE);	
...
//							}
							   ...
							   Enemy.get(i).kind == CoolEditDefine.Enemy_MMB4)
						{
							...
							if(Enemy.get(i).life<=0)
							{								
								Enemy.get(i).setState(Sprite.SPRITE_STATE_NONE);	
...
//								{			
									...
									effect.add(Effect.SMALLGOLDEN, (int)Enemy.get(i).x, (int)Enemy.get(i).y-SpriteLibrary.GetH(Enemy.get(i).kind)/2, num);																																			
//								}
//								else
...
//								gameMain.gameMission.addMonsterDeadTime(gameMain);
							...
							else
							{
								...
								{
									Enemy.get(i).changeAction(Sprite.Enemy_action12);
									
									Enemy.get(i).setState(Sprite.SPRITE_STATE_INJURE);
								}
								else
								{
									Enemy.get(i).changeAction(Sprite.Enemy_action14);
...
//								}
							...
							effect.add(Effect.FLOWER, (int)Enemy.get(i).x, (int)Enemy.get(i).y-SpriteLibrary.GetH(Enemy.get(i).kind)/2, 0);
						}
						...
						{							
							...
							Enemy.get(i).changeAction(Sprite.Enemy_action05);
						...
						else if(Enemy.get(i).kind==CoolEditDefine.Enemy_YGYJF)
						{
							...
							Enemy.get(i).changeAction(Sprite.Enemy_action03);
							
							Enemy.get(i).setState(Sprite.SPRITE_STATE_STAND);
...
//								{
									if(Enemy.get(i).life<=0)
									{
										...
										{											
											if(Enemy.get(i).state==Sprite.SPRITE_STATE_REVIVAL)
											{												
												Enemy.get(i).changeAction(Sprite.Enemy_action02);
...
//												gameMain.gameNumber += SpriteLibrary.GetNumber(Enemy.get(i).kind);
											...
											{												
												Enemy.get(i).changeAction(Sprite.Enemy_action21);
											
												Enemy.get(i).setState(Sprite.SPRITE_STATE_REVIVAL);
...
//											gameMain.gameMission.addMonsterDeadTime(gameMain);
										...
										{													
											...
											if(bullet.special==Sprite.PLAYER_SPECIAL_3)//弹飞
											{
												if(Enemy.get(i).kind!=CoolEditDefine.Enemy_MM&&
												Enemy.get(i).kind!=CoolEditDefine.Enemy_MMJS&&
												Enemy.get(i).kind!=CoolEditDefine.Enemy_SHZY&&
									...
									{																						
										if(Enemy.get(i).dizzinessTime<=0)
										{
											if(Enemy.get(i).kind == CoolEditDefine.Enemy_SHHT)
											{
												...
												{
													Enemy.get(i).changeAction(Sprite.Enemy_action15);																								
												}
...
//										}
										...
										if(bullet.special==Sprite.PLAYER_SPECIAL_2)//眩晕
										{
											if(Enemy.get(i).kind!=CoolEditDefine.Enemy_HZXJ&&
												Enemy.get(i).kind!=CoolEditDefine.Enemy_MEGICWATER&&
												...
												Enemy.get(i).kind!=CoolEditDefine.Enemy_YGYYM)
											{
												...
												if(Enemy.get(i).life>0)
												{
													effect.add(Enemy.get(i), CoolEditDefine.Effect_DIZZINESS, (int)Enemy.get(i).x, (int)Enemy.get(i).y-SpriteLibrary.GetH(Enemy.get(i).kind), Sprite.SPRITE_STATE_NORMAL, Enemy.get(i).dizzinessTime);
										...
										else if(bullet.special==Sprite.PLAYER_SPECIAL_4)//点燃
										{
											if(Enemy.get(i).kind!=CoolEditDefine.Enemy_HZXJ&&
												Enemy.get(i).kind!=CoolEditDefine.Enemy_MEGICWATER&&
												Enemy.get(i).kind!=CoolEditDefine.Enemy_MM&&
										...
										else if(bullet.special==Sprite.PLAYER_SPECIAL_1)//减速
										{	
...
//								}
								...
								effect.add(CoolEditDefine.Effect_ATTACK2, (int)Enemy.get(i).x, (int)Enemy.get(i).y-SpriteLibrary.GetH(Enemy.get(i).kind)/2, Sprite.SPRITE_STATE_NORMAL, 50, 1f);																		
							
								addDamageNumberAnimation((int)Enemy.get(i).x, (int)Enemy.get(i).y, attackNumber);														
...
//									{
										...
										if(Enemy.get(i).state==Sprite.SPRITE_STATE_DEAD||
										   Enemy.get(i).state==Sprite.SPRITE_STATE_NONE)
										{
								...
								else if(bullet.getActionName()==3)
								{
									...
									{										
										   ...
										   bullet.kind==CoolEditDefine.Player_YC_3)
										{
											if(bullet.kind==CoolEditDefine.Player_YC)
											{
												bullet.setCollisionRect(
														bullet.x-SpriteLibrary.GetW(bullet.kind)/2*2,
														bullet.y-SpriteLibrary.GetH(bullet.kind)/2*2,
														bullet.x+SpriteLibrary.GetW(bullet.kind)/2*2, 
														bullet.y+SpriteLibrary.GetH(bullet.kind)/2*2);														
											}
											else if(bullet.kind==CoolEditDefine.Player_YC_2)
											{
												bullet.setCollisionRect(
														bullet.x-SpriteLibrary.GetW(bullet.kind)/2*3,
														bullet.y-SpriteLibrary.GetH(bullet.kind)/2*3,
														bullet.x+SpriteLibrary.GetW(bullet.kind)/2*3, 
														bullet.y+SpriteLibrary.GetH(bullet.kind)/2*3);														
											}
											else if(bullet.kind==CoolEditDefine.Player_YC_3)
											{
												bullet.setCollisionRect(
														bullet.x-SpriteLibrary.GetW(bullet.kind)/2*5,
														bullet.y-SpriteLibrary.GetH(bullet.kind)/2*5,
														bullet.x+SpriteLibrary.GetW(bullet.kind)/2*5, 
														bullet.y+SpriteLibrary.GetH(bullet.kind)/2*5);														
											}
											
											bullet.special = Sprite.PLAYER_SPECIAL_2;
										   ...
										   bullet.kind==CoolEditDefine.Player_LJ_3)
										{
											...
											{
												bullet.setCollisionRect(
														bullet.x-SpriteLibrary.GetW(bullet.kind)/2*4,
														bullet.y-SpriteLibrary.GetH(bullet.kind)/2*4,
														bullet.x+SpriteLibrary.GetW(bullet.kind)/2*4, 
														bullet.y+SpriteLibrary.GetH(bullet.kind)/2*4);														
											}
											
											bullet.special = Sprite.PLAYER_SPECIAL_4;
										   ...
										   bullet.kind==CoolEditDefine.Player_NG_3)
										{
											...
											if(bullet.state!=Sprite.SPRITE_STATE_NONE)
											{
												i = -1;
...
//								}	
	...
	private void spriteFlyMove(Sprite enemy)
	{	
		if(enemy.isFly)
		{
			enemy.jiaodu += 30;
			...
			//反弹
			if(enemy.x-SpriteLibrary.GetW(enemy.kind)/2<0)
			{
				enemy.isFlyMoveDegree = 90 - (enemy.isFlyMoveDegree-90);
				
				enemy.jiaodu = -enemy.jiaodu;							
			}
			else if(enemy.x+SpriteLibrary.GetW(enemy.kind)/2>GameConfig.GameScreen_Width)
			{
			...
			if(enemy.y<0||enemy.x<0||enemy.x>GameConfig.GameScreen_Width)
			{
				...
				enemy.setState(Sprite.SPRITE_STATE_NONE); 
			}
		}
	}
