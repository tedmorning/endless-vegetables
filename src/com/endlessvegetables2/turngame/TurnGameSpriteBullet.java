...
public class TurnGameSpriteBullet {
	...
	private ArrayList<TurnGameSprite> spriteBulletList;
	
	private TurnGameEffect effect;
	
	public TurnGameSpriteBullet()
	{
		spriteBulletList = new ArrayList<TurnGameSprite>();
	...
	private void setSpriteBullet(TurnGameSprite sprite, int x, int y, float degree, int speed)
	{		
		sprite.setXY(x, y);
		...
		sprite.state = TurnGameSprite.SPRITE_STATE_NORMAL;
	...
	{				
		for(int i=0;i<spriteBulletList.size();i++)
		{
			if(!spriteBulletList.get(i).isMove&&
			    spriteBulletList.get(i).state == TurnGameSprite.SPRITE_STATE_NONE)
			{
				spriteBulletList.remove(i);
			}
		}
		
		TurnGameSprite bullet = new TurnGameSprite();			
		bullet.initSprite(kind, 0, 0, TurnGameSprite.SPRITE_STATE_NORMAL);
	...
	private void spriteBulletMove(TurnGameSprite sprite, TurnGameMain gameMain)
	{				
		if(sprite.isMove)
		{		
...
//			{
			   ...
			   sprite.kind!=CoolEditDefine.Player_NG_3))		
			{
				...
				{
					//·´µ¯
					if(sprite.x-TurnGameSpriteLibrary.GetW(sprite.kind)/2<0)
					{
...
//						GameMedia.playSound(R.raw.yx014, 0);
					...
					else if(sprite.x+TurnGameSpriteLibrary.GetW(sprite.kind)/2>GameConfig.GameScreen_Width)
					{
					...
					{			
						...
						while(num>0)
						{
							...
							if(sprite.x<=TurnGameSpriteLibrary.GetW(sprite.kind)/2)
								x = (int)(TurnGameSpriteLibrary.GetW(sprite.kind)/2);
							else if(sprite.x>=GameConfig.GameScreen_Width-TurnGameSpriteLibrary.GetW(sprite.kind)/2)
								x = (int)(GameConfig.GameScreen_Width-TurnGameSpriteLibrary.GetW(sprite.kind)/2);
...
//						sprite.setState(Sprite.SPRITE_STATE_NONE);
						...
						effect.add(CoolEditDefine.Effect_BOMB, (int)sprite.x, (int)sprite.y-TurnGameSpriteLibrary.GetH(sprite.kind)/2, TurnGameSprite.SPRITE_STATE_NORMAL, 0, 1f);
...
//					sprite.setState(Sprite.SPRITE_STATE_NONE);
					...
					effect.add(CoolEditDefine.Effect_BOMB, (int)sprite.x, (int)sprite.y-TurnGameSpriteLibrary.GetH(sprite.kind)/2, TurnGameSprite.SPRITE_STATE_NORMAL, 0, 0.5f);
...
//							gameMain.combo.comboNumberClaer();
	...
	{				
		...
		for(int i=0;i<spriteBulletList.size();i++)
		{
			if(spriteBulletList.get(i).state!=TurnGameSprite.SPRITE_STATE_NONE)
			{
				   ...
				   spriteBulletList.get(i).getActionName()==6)
				{
					...
					if(spriteBulletList.get(i).lifeTime%10==0)
					{
	...
	public TurnGameSprite getSpriteBullet(int id)
	{
	...
	public void changeSameGroupIDClearCombo(TurnGameMain gameMain)
	{			
		for(int i=0;i<spriteBulletList.size();i++)
		{			
			if(spriteBulletList.get(i).kind==CoolEditDefine.Player_WD||
